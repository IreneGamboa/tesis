...
if(cluster.isMaster){
	var numWorkers = require('./config/server_config').nclusters;
	console.log(`Master ${process.pid} is running with ${numWorkers}`);
	...
	cluster.on('exit', function(worker, code, signal){
		console.log('[LOG]: Worker ' + worker.process.pid + ' died with code: ' + code + ', and signal: ' + signal);
		console.log('[LOG]: Starting a new worker');
		cluster.fork();
	});
}
else{
